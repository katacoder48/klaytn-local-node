...
(async () => {
    ...
    // 3. read public address or create test keys(privateKey) to receive fund
    const testAddreses = await readOrCreateTestAddresses();

    // 4. receive Fund per TestAddresses
    const amount = 10000;
    console.log(`FUND ADDRESS : ${fundAddress} 에서 아래 계좌들로 ${amount} 클레이 배분`)
    await Promise.all(testAddreses.map((address:string) =>
        receiveFund(address, fundAddress, amount)
    ));
})();